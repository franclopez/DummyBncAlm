...
*/(function(window, angular, undefined) {
  ...
  function $IdleProvider() {
    ...
    var options = {
      ...
      firtsWarning: 20,//MODIFICADO primer advertencia///////////////
      secondWarning: 10,//MODIFICADO segunda advertencia///////////////
    ...
    //MODIFICADO 1ER Y SEGUNDA ADVERTENCIAS///////////////
    this.firtsWarning = function(seconds) {
      if (seconds < 0) throw new Error("warning must be a value in seconds, greater than 0.");

      options.firtsWarning = seconds;
    };
    
    this.secondWarning = function(seconds) {
      if (seconds < 0) throw new Error("warning must be a value in seconds, greater than 0.");

      options.secondWarning = seconds;
    };
    //MODIFICADO FN///////////////
    ...
    this.$get = ['$interval', '$log', '$rootScope', '$document', '$keepalive', function($interval, $log, $rootScope, $document, $keepalive){
      ...
      function toggleState(){
        ...
        if(state.idling){
          state.firtsWarning = options.firtsWarning;
          state.secondWarning = options.secondWarning;
          stopKeepalive();
          state.countdown = options.warningDuration;
          countdown();
          state.warning = $interval(countdown, 1000, options.warningDuration, false);//fnc millCadaRep CantARep false
        }else{
          startKeepalive();
        }
      ...
      function countdown() {
        ...
        //MODIFICADO se agregan 2 countdown para las advertencias
        if (state.countdown == state.firtsWarning) {
          console.log('$idleFirtsWarning');
          warningMsg('$idleFirtsWarning');
        }
        
        if (state.countdown == state.secondWarning) {
          console.log('$idleSecondWarning');
          warningMsg('$idleSecondWarning');
        }
        //MODIFICADO FN ///////////////
      ...
      function warningMsg(idWarn){
        stopKeepalive();
        $rootScope.$broadcast(idWarn);
      }
      ...
      var svc = {
        ...
        watch: function() {
          ...
          //state.fWarn = $interval(toggleState, options.idleDuration * 1000, 0, false);
          //state.sWarn = $interval(toggleState, options.idleDuration * 1000, 0, false);
        },
        unwatch: function() {
          $interval.cancel(state.idle);
          $interval.cancel(state.warning);
