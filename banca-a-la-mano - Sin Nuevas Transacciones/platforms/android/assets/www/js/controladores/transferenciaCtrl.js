    ...
    */
    ...
    $scope.validarCampos = function(estado){
        if($scope.tipoCuentaValido && $scope.envioDinero.valorTransferir != '' && $scope.envioDinero.cuentaDestino != ''){
            ...
            $scope.form.envioDinero.valorTransferir = 
            transferenciaSol.headerRQ = SolHeader.header($rootScope.conf.BEAN_SOLICITUD_ENVIO_DINERO);            
    ...
    $scope.confirmar = function(estado){
        try{
            if($scope.form.pin != undefined){
                transferenciaSol.headerRQ = SolHeader.header($rootScope.conf.BEAN_CONFIRMACION_ENVIO_DINERO);
