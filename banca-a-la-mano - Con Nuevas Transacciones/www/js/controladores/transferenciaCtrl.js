...
.controller('TransferenciaRealizarCtrl',function($scope, $state, $ionicPopup, $rootScope, OpcionesTransaccionRadio, TransferenciaRealizarTxt, BncALMDatos, Conexion, SolHeader, SolPinTransferencia){//TimeOut
    ...
    $scope.envioDinero= {
        ...
        valorTransferir : '',
        cuenta: {
            minLength: $rootScope.conf.INPUT_CUENTA_MIN,
            maxLength: $rootScope.conf.INPUT_CUENTA_MAX
        },
        valor: {
            minLength: $rootScope.conf.INPUT_VAL_TRANS_MIN,
            maxLength: $rootScope.conf.INPUT_VAL_TRANS_MAX
        }
    };
...
//    };
    */
    ...
    $scope.actualizarBoton = function(){        
        if($scope.envioDinero.cuentaDestino != '' && $scope.envioDinero.valorTransferir!= '' && $scope.envioDinero.valorTransferir != 0 ){
            
            if($scope.formularioRadio[0].checked || $scope.formularioRadio[1].checked){
                $scope.botonTransaccion = {
                    color: 'azul',
                    contenido: 'Continuar'
                };
    ...
    $scope.validarNumeros = function() {
        ...
        ////Activar boton continuar
        if($scope.envioDinero.cuentaDestino != '' && $scope.envioDinero.cuentaDestino != 0 && $scope.envioDinero.valorTransferir!= '' && $scope.envioDinero.valorTransferir != 0 ){
...
.controller('TransferenciaConfirmarCtrl',function($state, $scope, $rootScope, $ionicPopup, TransferenciaConfirmarTxt, SolHeader, SolTransferencia, Conexion){//TimeOut
    ...
    $scope.patronValidacion = /^[0-9]+$/;
    $scope.form = {
        minLength: $rootScope.conf.INPUT_PIN_MIN,
        maxLength: $rootScope.conf.INPUT_PIN_MAX
    };
    ...
    $scope.validarNumeros = function() {
        ...
        if($scope.form.pin != 0 && $scope.form.pin != ''){
            $scope.botonConfirmar = {
                color: 'azul',
                contenido: 'Confirmar'
            };            
